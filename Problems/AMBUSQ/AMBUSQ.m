function [fn, FnVar, FnGrad, FnGradCov, constraint, ConstraintCov, ConstraintGrad, ConstraintGradCov] = AMBUSQ(x, runlength, problemRng, seed, ProblemInstance)
...
if (sum(x < 0) > 0) || (sum(x > 1) > 0) || (sum(size(x) ~= [1, 2 * nAmbulances])>0) || (runlength <= 0) || (seed <= 0) || (round(seed) ~= seed)
    ...
    vf = velfk/30;
    vs = velsk/30;  % Velocities scaled to relate to the unit square
    ...
    for replication = 1:nDays
        ...
        while j~=NumCalls + 1
            u = rand(3,1);
            % Shift to make density nonnegative
            ARcoeff = max(callmodex, 1-callmodex) + max(callmodey, 1-callmodey);
            if ARcoeff * u(3) <= ARcoeff - (abs(u(1)-callmodex)+abs(u(2)-callmodey))
        ...
        % last call responded by each ambulance, i.e. 
        % col. 1 = Time at which last call was finished (travel+service) for ambulance 1, 
        % col. 2 = X location of last call,
        % col. 3 = Y location of last call, 
        % col. 4 = X location of its base, and
        % col. 5 = Y location of its base.
