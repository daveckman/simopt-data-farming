...
%        ProblemInstance
%              A cell containing parameters describing the problem instance
%        SolverParam
%              A cell containing parameters of the solver
%        
% OUTPUT
%        Ancalls
...
function [Ancalls, A, AFnMean, AFnVar, AFnGrad, AFnGradCov, ...
    ...
    solverRng, ProblemInstance, SolverParam)
...
CellSize = size(SolverParam);
if (CellSize(1) ~= 1) || (CellSize(2) ~= nparam)
  fprintf('Input parameter cell to STRONG should be a cell of %d components. \n', nparam);
  return;

else

    % Need to check constraints on components of SolverParam (e.g.,
    % nonnegativity, integer)
    
    for param_index = 1:nparam
        if isempty(SolverParam{param_index})
            SolverParam{param_index} = defaults{param_index};
        end
    end
end
...
r = SolverParam{1};
delta_threshold = SolverParam{2};
delta_T = SolverParam{3};
eta_0 = SolverParam{4};
eta_1 = SolverParam{5};
gamma1 = SolverParam{6};
gamma2 = SolverParam{7};
...
[minmax, dim, ~, ~, VarBds, ~, ~, solution, budget, ~, ~, ~] = probstructHandle(1, ProblemInstance);
...
[Q_bar_old, fn0varcurrent, ~, ~, ~, ~, ~, ~] = probHandle(solution, r, problemRng, problemseed, ProblemInstance);
Bspent = Bspent + r;
...
record_index = 2;
 ...
 while Bspent <= budget
     ...
     if delta_T > delta_threshold    %stage I
         ...
         [Grad, Hessian] = FiniteDiff(solution, Q_bar_old, BdsCheck, 1, r, probHandle, problemRng, problemseed, dim, delta_T, VarBds, minmax, ProblemInstance);
         ...
         [Q_bar_new, newVar, ~, ~, ~, ~, ~, ~] = probHandle(new_solution, r, problemRng, problemseed, ProblemInstance);
         Bspent = Bspent + r;
         ...
         [Grad, Hessian] = FiniteDiff(solution, Q_bar_old, BdsCheck, 2, r, probHandle, problemRng, problemseed, dim, delta_T, VarBds, minmax, ProblemInstance);
         Bspent = Bspent + NumOfEval*r;
         ...
         if rho < eta_0 || (Q_bar_old - Q_bar_new) <= 0 || (r_old - r_new) <= 0
            ...
            while sum(result_solution ~= solution) == 0  %was while result_solution==solution
                ...
                [G, H] = FiniteDiff(solution, rr_old, BdsCheck, 2, (sub_counter + 1)*r, probHandle, problemRng, problemseed, dim, delta_T, VarBds, minmax, ProblemInstance);
                ...
                [Q_b_new, var, ~, ~, ~, ~, ~, ~] = probHandle(try_solution, r + ceil(sub_counter^1.01), problemRng, problemseed, ProblemInstance);
                Q_b_new = -minmax*Q_b_new;
                Bspent = Bspent + r + ceil(sub_counter^1.01); %%%
                [dummy, ~, ~, ~, ~, ~, ~, ~] = probHandle(solution, ceil(sub_counter^1.01) - ceil((sub_counter - 1)^1.01), problemRng, problemseed, ProblemInstance); %%%
                dummy = -minmax*dummy;
...
AFnVar = AFnVar(1:record_index);
 ...
 %% Helper Function FiniteDiff
    function [Grad, Hessian] = FiniteDiff(solution, fn, BdsCheck, stage, runlen, probHandle, problemRng, problemseed, dim, delta_T, VarBds, minmax, ProblemInstance)
        ...
        for i = 1:dim
            ...
            if BdsCheck(i) ~= -1
                [fn1, ~, ~, ~, ~, ~, ~, ~] = probHandle(x1, runlen, problemRng, problemseed, ProblemInstance);
                fn1 = -minmax*fn1;
                FnPlusMinus(i,1) = fn1; % first column is f(x+h,y)
            end
            
            if BdsCheck(i) ~= 1
                [fn2, ~, ~, ~, ~, ~, ~, ~] = probHandle(x2,runlen, problemRng, problemseed, ProblemInstance);
                fn2 = -minmax*fn2;
        ...
        if stage == 2
            ...
            for i = 1:dim
                ...
                elseif BdsCheck(i) == 1
                    ...
                    %check budget
                    [fn3, ~, ~, ~, ~, ~, ~, ~] = probHandle(x3, runlen, problemRng, problemseed, ProblemInstance);
                    fn3 = -minmax*fn3;
                ...
                elseif BdsCheck(i) == -1
                    ...
                    %check budget
                    [fn4, ~, ~, ~, ~, ~, ~, ~] = probHandle(x4, runlen, problemRng, problemseed, ProblemInstance);
                    fn4 = -minmax*fn4;
                ...
                for j = i+1:dim
                    if BdsCheck(i)^2 + BdsCheck(j)^2 == 0 % neither x nor y on boundary
                        ...
                        %check budget
                        [fn5, ~, ~, ~, ~, ~, ~, ~] = probHandle(x5, runlen, problemRng, problemseed, ProblemInstance);
                        fn5 = -minmax*fn5;
                        % f(x-h,y-k)
                        x6 = solution;
                        ...
                        %check budget
                        [fn6, ~, ~, ~, ~, ~, ~, ~] = probHandle(x6, runlen, problemRng, problemseed, ProblemInstance);
                        fn6 = -minmax*fn6;
